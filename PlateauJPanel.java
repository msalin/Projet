...
public class PlateauJPanel extends JPanel implements MouseListener {
	/** NEW !! */
	private static final int BORD = 107;
	/** NEW !! */
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < plateau.size; i++) {
			for (int j = 0; j < plateau.size; j++) {
				...
				g.drawImage(getPionImage(c.getCouleur()), i*TAILLE_CASE + BORD, j*TAILLE_CASE+ BORD, TAILLE_CASE, TAILLE_CASE, null);
			}
		}
	}
	...
	public void mouseReleased(MouseEvent e) {
		/** NEW !! */
		int caseX = (e.getX()-BORD)/TAILLE_CASE;
		int caseY = (e.getY()-BORD)/TAILLE_CASE;
		if (caseX >=0 && caseX < plateau.size
		&& 	caseY >=0 && caseY < plateau.size
				) {
